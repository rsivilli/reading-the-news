...
def get_audio_files(article_id:int):
    return ArticleAudio.objects.filter(article__id=article_id).all().order_by("paragraph_number")
def get_image_files(article_id:int):
    return ArticleImage.objects.filter(article__id=article_id).all().order_by("paragraph_number")

def assemble_article_content(
        article_id:int, build_directory:str="tmp",save_directory:str = "article_video"
):
    article = Article.objects.get(pk=article_id)
    audio_files = get_audio_files(article_id)
    image_files = get_image_files(article_id)
    if len(audio_files) != len(image_files):
        raise ValueError(f"There are a difference number of audio files ({len(audio_files)}) than image files ({len(image_files)}) for artcile {article_id}")
