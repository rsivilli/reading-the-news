...
class SpeechGenerator(ContentGenerator):
    def __init__(
        self,
        processor="suno/bark",
        model="suno/bark",
        semantic_min_eos_p=0.05,
        semantic_temperature=0.98,
    ) -> None:
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model: BarkModel = AutoModel.from_pretrained(model).to(
            self.device
        )
        self.processor: BarkProcessor = AutoProcessor.from_pretrained(
            processor
        )
    ...
    def generate_audio(self, text, preset, silence_duration=0.25):
        ...
        silence = np.zeros(int(silence_duration * sampling_rate), np.float32)
    ...
    def generate_and_save_audio(
        ...
        for sentence in nltk.sent_tokenize(paragraph):
            pieces += self.generate_audio(sentence, preset)
    ...
    def generate_audio_for_article(
        ...
        for p in tqdm(article.content.split("\n")):
            self.generate_and_save_audio(
                article, count, p, preset, audio_folder
            )
            count += 1

    def replace_audio_for_article(
        self,
        article_id,
        paragraph_numbers: int | list[int],
        preset="v2/en_speaker_9",
        audio_folder="article_audio",
    ):
        article = Article.objects.filter(pk=article_id).first()
        ...
        for p in tqdm(paragraph_numbers):
            existing_audio = ArticleAudio.objects.filter(
                article__id=article_id, paragraph_number=p
            ).first()
            if existing_audio is None:
                print(
                    f"There is not an existing audio for Article {article_id}, Paragraph {p}"  # noqa: E501
                )
            else:
    ...
    def generate_content(self, article_id: int, args: dict):
        self.generate_audio_for_article(article_id=article_id, **args)
