...
def outlet_detail(request,outlet_id:int):
    ...
    articles = Paginator(Article.objects.filter(outlet__id = outlet_id).order_by("-publish_date").all(),10)
...
def article_detail(request,article_id:int):
    ...
    images = ArticleImage.objects.filter(article__id=article_id).order_by("paragraph_number").all()
    audio = ArticleAudio.objects.filter(article__id=article_id).order_by("paragraph_number").all()
...
def articles(request,outlet_id:int):
    ...
    outlet = get_object_or_404(NewsOutlet,pk=outlet_id)
    articles = Paginator(Article.objects.filter(outlet__id=outlet_id).all(),10)
    context = {
        "articles":articles,
        "outlet": outlet
    }
