...
class BreakingDefenseArticle(Article):
    ...
    def __init__(self, soup_item:BeautifulSoup) -> None:
        ...
        [div.decompose() for div in content.find_all("div", class_=re.compile("sponsored|related|postFooter"))]
...
class SpaceNewsArticle(Article):
    ...
    def __init__(self, soup_item:BeautifulSoup) -> None:
        self._paragraphs = []
        content = soup_item.find_all("article",class_=re.compile("\spost\s"))[0].find_all("div",class_="entry-content")[0]
...
def get_article(article_url:str,article_class_name="BreakingDefenseArticle"):
    ...
    if article_class is None:
        raise ValueError(f"Article Parser of with name {article_class_name} does not exist")
...
if __name__ == "__main__":
    ...
    for channel in channels:
        for item in channel.items:
            #should check to see if we know about this article before grabbing it
